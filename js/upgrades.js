    ...
    for (const def of WEAPON_LIST) {
      if (!p.hasWeapon(def.id)) {
        pool.push({
          ...
          desc: def.desc(1), level: 1, isNew: true, weight: 0.85,
        });
      }
    }
  }

  // 3) Upgrade existing passives.
  for (const def of PASSIVE_LIST) {
    ...
    if (lvl > 0 && lvl < def.max) {
      pool.push({
        ...
        desc: def.desc(lvl + 1), level: lvl + 1, isNew: false, weight: def.rarity,
      });
    }
  }
    ...
    for (const def of PASSIVE_LIST) {
      if (!(p.passives[def.id] > 0)) {
        pool.push({
          ...
          desc: def.desc(1), level: 1, isNew: true, weight: def.rarity * 0.9,
        });
      }
    }
  }
